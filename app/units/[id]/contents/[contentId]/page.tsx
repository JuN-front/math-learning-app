...
export default function ContentPage({ params }: { params: { id: string; contentId: string } }) {
  ...
  const markProgress = async (field: string) => {
    await fetch(`/api/contents/${params.contentId}/progress`, {
      method: 'POST',
  ...
  const handleAssignmentUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    form.append('content_id', params.contentId);
  ...
  const handleDeleteAssignment = async () => {
    ...
    await fetch(`/api/assignments?content_id=${params.contentId}`, { method: 'DELETE' });
    fetchContent();
  };
      ...
      <div className="min-h-screen" style={{ background: 'var(--surface)' }}>
        ...
        <main className="max-w-4xl mx-auto px-6 py-8 text-center">
          <div className="card p-12 mt-8">
            ...
            <Link href={`/units/${params.id}`} className="btn-primary inline-flex items-center gap-2">
    ...
    <div className="min-h-screen" style={{ background: 'var(--surface)' }}>
      ...
      <main className="max-w-4xl mx-auto px-6 py-8">
        ...
        <div className="flex items-center gap-2 text-sm text-gray-500 mb-5">
          ...
          <span>/</span>
          <Link href={`/units/${params.id}`} className="hover:text-blue-600 flex items-center gap-1">
            <ChevronLeft size={14} /> 単元
          </Link>
          <span>/</span>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <div className="text-sm text-gray-500">
                      {prog?.video_watched ? '視聴済み ✓' : '未視聴'}
                    </div>
                  </div>
                </div>
                {content.video_path ? (
                  <Link
                    href={`/units/${params.id}/contents/${params.contentId}/video`}
                    onClick={() => markProgress('video_watched')}
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <div className="text-sm text-gray-500">
                      {prog?.textbook_read ? '閲覧済み ✓' : '未閲覧'}
                    </div>
                  </div>
                </div>
                {content.textbook_path ? (
                  <Link
                    href={`/units/${params.id}/contents/${params.contentId}/pdf?type=textbook`}
                    onClick={() => markProgress('textbook_read')}
                    className="btn-primary text-sm flex items-center gap-2"
                  >
          ...
          {content.has_assignment && (
            <div className="card p-5">
              <div className="flex items-start gap-3">
                <div className="bg-orange-100 rounded-lg p-2.5 shrink-0">
                ...
                <div className="flex-1">
                  ...
                  {content.assignment_path && (
                    <div className="mb-3">
                      <Link
                        href={`/units/${params.id}/contents/${params.contentId}/pdf?type=assignment`}
                      ...
                      <div className="flex items-center justify-between">
                        <div>
                          ...
                          <div className="text-xs text-gray-400">
                            {new Date(content.assignment.submitted_at).toLocaleString('ja-JP')}
                          </div>
                        </div>
                        <button
                  ...
                  {content.answer_path && prog?.assignment_submitted && (
                    <div className="mt-3">
                      <Link
                        href={`/units/${params.id}/contents/${params.contentId}/pdf?type=answer`}
        ...
        {/* Back */}
        <div className="mt-8">
          <Link href={`/units/${params.id}`} className="text-sm text-blue-600 hover:underline flex items-center gap-1">
